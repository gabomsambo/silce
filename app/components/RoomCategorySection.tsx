        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Link href={`/rooms/${featured.slug}`} className="group relative rounded-2xl overflow-hidden border border-gray-200">
            <div className="relative h-72">
              <Image src={cardPhoto(featured.images[0]?.src || "/placeholder.svg")} alt={t(featured.titleKey)} fill className="object-cover group-hover:scale-105 transition-transform duration-300" />
            </div>
            <div className="p-5">
            ...
            {units.slice(1).map((u) => (
              <Link key={u.slug} href={`/rooms/${u.slug}`} className="group relative rounded-2xl overflow-hidden border border-gray-200">
                <div className="relative h-56">
                  <Image src={cardPhoto(u.images[0]?.src || "/placeholder.svg")} alt={t(u.titleKey)} fill className="object-cover group-hover:scale-105 transition-transform duration-300" />
                </div>
                <div className="p-4">
                  <div className="flex items-center justify-between mb-1">
